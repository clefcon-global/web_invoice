...
import {
  SCHEMA_VERSION, STORAGE_PREFIX, appendDocument, exportSnapshot, importSnapshot, loadCounters,
  loadCustomers, loadDocuments, loadOwner, loadProducts, saveCounters, saveCustomers, saveOwner, saveProducts,
} from './storage.js';
...
test('round-trips every stored record and document counters', () => {
  ...
  expect(loadDocuments()).toEqual([document]); saveCounters({ invoice: 3, receipt: 2 }); expect(loadCounters()).toEqual({ invoice: 3, receipt: 2 });
});
...
test('exports and imports every field, rejecting unsupported schemas', () => {
  saveOwner({ businessName: 'Test Traders', addressLines: [], paymentLines: [] }); saveCustomers([{ id: 'c', name: 'Mx. Test Customer' }]); saveProducts([{ id: 'p', description: 'Test Product', unitPrice: 1 }]); saveCounters({ invoice: 1, receipt: 2 });
